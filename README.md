...
The **Codes** directory provides working codes together with pretrained weights for RAFT and FlowFormer. Pretrained weights for the homography network can be found in the **Releases** section. The **train_homo_opt.ipynb** notebook contains all required functions for training, eveluation and 3D scene point traingulation. Run the **evaluation.ipynb** to benchmark against *Lucas-Konade*, *RAFT*, and *FlowFormer*. A dataset that includes camera intrinsic and extrinsic parameters for the images of the Kavita and Lalith Bahl Smart bridge at the University of Illinois Urbana-Champaign is also provided.
